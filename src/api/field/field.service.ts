...
export class FieldService extends SqlService {
	...
	async dropOption({ user, ...payload }): Promise<any> {
		...
		try {
			...
			await this.fieldFieldFieldOptionRepository.delete({ fieldFieldOptionId: payload['id'] });
			await this.fieldFieldOptionRepository.delete({ id: payload['id'] });
	...
	async createOption({ 
		...
		try {
			...
			const fieldFieldOption = await this.fieldFieldOptionRepository.save({
				fieldId: id,
			...
			output['fieldFieldOptions'] = [ fieldFieldOption ];
