...
export class FormStatusController {
	constructor(
		private readonly formStatusService: FormStatusService,
	...
	async many(payload) {
		try {
			const many = await this.formStatusService.many({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_FORM_STATUS_MANY'] ],
					isRequired: true,
				}),
	...
	async one(payload) {
		try {
			const output = await this.formStatusService.one({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_FORM_STATUS_ONE'] ],
					isRequired: true,
				}),
				relations: Validators.obj('relations', payload['relations']),
				select: Validators.obj('select', payload['select']),
	...
	async drop(payload) {
		try {
			await this.formStatusService.drop({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_FORM_STATUS_DROP'] ],
					isRequired: true,
				}),
				id: Validators.id('id', payload['id'], {
					isRequired: true,
	...
	async dropMany(payload) {
		try {
			await this.formStatusService.dropMany({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_FORM_STATUS_DROP_MANY'] ],
					isRequired: true,
				}),
	...
	async create(payload) {
		try {
			const output = await this.formStatusService.create({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_FORM_STATUS_CREATE'] ],
					isRequired: true,
				}),
				id: Validators.id('id', payload['id']),
	...
	async update(payload) {
		try {
			await this.formStatusService.update({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_FORM_STATUS_UPDATE'] ],
					isRequired: true,
				}),
				id: Validators.id('id', payload['id']),
