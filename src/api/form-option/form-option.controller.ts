...
export class FormOptionController {
	...
	async many(payload) {
		try {
			const many = await this.formOptionService.many({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_FORM_OPTION_MANY'] ],
					isRequired: true,
				}),
	...
	async one(payload) {
		try {
			const output = await this.formOptionService.one({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_FORM_OPTION_ONE'] ],
					isRequired: true,
				}),
				relations: Validators.obj('relations', payload['relations']),
				select: Validators.obj('select', payload['select']),
	...
	async drop(payload) {
		try {
			await this.formOptionService.drop({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_FORM_OPTION_DROP'] ],
					isRequired: true,
				}),
				id: Validators.id('id', payload['id'], {
					isRequired: true,
	...
	async dropMany(payload) {
		try {
			await this.formOptionService.dropMany({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_FORM_OPTION_DROP_MANY'] ],
					isRequired: true,
				}),
	...
	async create(payload) {
		try {
			const output = await this.formOptionService.create({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_FORM_OPTION_CREATE'] ],
					isRequired: true,
				}),
				id: Validators.id('id', payload['id']),
	...
	async update(payload) {
		try {
			await this.formOptionService.update({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_FORM_OPTION_UPDATE'] ],
					isRequired: true,
				}),
				id: Validators.id('id', payload['id']),
