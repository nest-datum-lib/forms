...
export class ContentStatusController {
	...
	async many(payload) {
		try {
			const many = await this.contentStatusService.many({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_CONTENT_STATUS_MANY'] ],
					isRequired: true,
				}),
	...
	async one(payload) {
		try {
			const output = await this.contentStatusService.one({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_CONTENT_STATUS_ONE'] ],
					isRequired: true,
				}),
				relations: Validators.obj('relations', payload['relations']),
				select: Validators.obj('select', payload['select']),
	...
	async drop(payload) {
		try {
			await this.contentStatusService.drop({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_CONTENT_STATUS_DROP'] ],
					isRequired: true,
				}),
				id: Validators.id('id', payload['id'], {
					isRequired: true,
	...
	async dropMany(payload) {
		try {
			await this.contentStatusService.dropMany({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_CONTENT_STATUS_DROP_MANY'] ],
					isRequired: true,
				}),
	...
	async create(payload) {
		try {
			const output = await this.contentStatusService.create({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_CONTENT_STATUS_CREATE'] ],
					isRequired: true,
				}),
				id: Validators.id('id', payload['id']),
	...
	async update(payload) {
		try {
			await this.contentStatusService.update({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_FORMS_CONTENT_STATUS_UPDATE'] ],
					isRequired: true,
				}),
				id: Validators.id('id', payload['id']),
